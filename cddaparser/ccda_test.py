...
class CcdaDocumentTestCase(unittest.TestCase):
  ...
  def test_sample_ccda_files(self):
    ...
    for basename in os.listdir(TESTDATA_DIR):
      ...
      fr = open(os.path.join(TESTRESULTS_DIR, basename) + ".csv", 'w')
      self._test_to_csv(fp, fr)
      #fp.seek(0)
